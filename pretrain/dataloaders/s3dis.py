...
# class S3DISDataset_old(object):
#     def __init__(self, cvfold, data_path):
#         '''
#         Args:
#             cvfold: which fold as testing classes
#             data_path: store the block
#         '''
...
#
#         if cvfold == 0:
#             self.test_classes = [self.type2class[i] for i in self.fold_0] # [test class id]
#         elif cvfold == 1:
...
#         # always put clutter (12) in the base stage
#         self.train_classes.append(12)
#
...
#             class2scans = {k:[] for k in range(self.classes)} # {0: [], 1:[], 2:[], ......., 12:[]}
#
...
class S3DISDataset(object):
    def __init__(self, cvfold, data_path):
        ...
        self.types = self.type2class.keys()
        # long-tail split
        self.fold_0 = ['table', 'window', 'column', 'beam', 'board', 'sofa']  # choose fold 0 as the tail classes
        self.fold_1 = ['wall', 'ceiling', 'floor', 'clutter', 'bookcase', 'door', 'chair']
...
if __name__ == '__main__':
    dataset = S3DISDataset(0, '../datasets/S3DIS/blocks_bs1_s1')
    print('done')