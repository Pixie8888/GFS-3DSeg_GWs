...
class ScanNetDataset(object):
    def __init__(self, cvfold, data_path):
        ...
        self.fold_0 = ['sink', 'toilet', 'bathtub', 'shower curtain', 'picture', 'counter']  # last 6 as novel class
        self.fold_1 = ['refridgerator', 'desk', 'curtain', 'sofa', 'bookshelf', 'bed', 'table',
                       'otherfurniture', 'window', 'cabinet', 'door', 'chair', 'unannotated', 'floor', 'wall']
        ...
        all_classes = [i for i in range(0, self.classes)] # include class 0!
        self.train_classes = [c for c in all_classes if c not in self.test_classes]

        self.class2scans = self.get_class2scans()
        print('train: {}, test: {}'.format(self.train_classes, self.test_classes))
    def get_class2scans(self):
        ...
        if os.path.exists(class2scans_file):
            ...
            # # clapping to maximum 10000 scans
            # max_scan = 10000
            # for class_id in range(self.classes):
            #     if len(class2scans[class_id]) > max_scan:
            #         new_list = random.sample(class2scans[class_id], max_scan)
            #         class2scans[class_id] = new_list
            #         print('class: {} is reduced to {}'.format(class_id, max_scan))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cvfold', type=int, default=0, help='Fold left-out for testing in leave-one-out setting '
                                                              'Options: {0,1}')
    parser.add_argument('--data_path', type=str, default='../../datasets/ScanNet/blocks_bs1.0_s1.0', help='Directory to source data')
