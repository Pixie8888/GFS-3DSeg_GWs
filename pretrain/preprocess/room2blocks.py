...
if __name__ == '__main__':
    ...
    parser.add_argument('--train', action='store_true', help='processing train or test data npy')

    args = parser.parse_args()

    DATA_PATH = args.data_path # S3DIS/scenes/
    ...
    if args.train == True:
        ...
        SAVE_PATH = os.path.join(os.path.dirname(DATA_PATH), 'blocks_bs{0}_s{1}_test'.format(BLOCK_SIZE, STRIDE), 'data')
    if not os.path.exists(SAVE_PATH): os.makedirs(SAVE_PATH)

    if args.train == True:
        file_paths = glob.glob(os.path.join(DATA_PATH, 'train_data', '*.npy'))
        print('{} scenes to be split...'.format(len(file_paths)))
    else:
        file_paths = glob.glob(os.path.join(DATA_PATH, 'test_data', '*.npy'))
        print('{} scenes to be split...'.format(len(file_paths)))
