...
def collect_point_label(anno_path, out_filename, file_format='numpy'):
    ...
    for f in glob.glob(os.path.join(anno_path, '*.txt')):
        cls = os.path.basename(f).split('_')[0] # for each instace
        if cls not in CLASS_NAMES:  # note: in some room there is 'staris' class..
            cls = 'clutter'
        points = np.loadtxt(f) # read an object txt file
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/home/yating/Documents/3d_segmentation/S3DIS/visualize_s3dis/Stanford3dDataset_v1.2_Aligned_Version',
                        help='Directory to dataset')
    parser.add_argument('--train', action='store_true', help='collect training data or teating data') # default is False.
    ...
    if args.train == True:
        folders = ["Area_1", "Area_2", "Area_3", "Area_4", "Area_5"] # train datasets
    else:
        folders = ["Area_6"]
    ...
    if args.train == True:
        SAVE_PATH = os.path.join(DST_PATH, 'scenes', 'train_data')
    else:
