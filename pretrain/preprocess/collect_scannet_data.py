...
ROOT_DIR = '/home/yating/Documents/3d_segmentation/GFS_pcd_seg'
...
def collect_point_label(scene_path, scene_name, out_filename):
    ...
    for i in range(len(instance_segids)):
        ...
        # record number of point for each class
        class_point_dict[label] += instance_points.shape[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', default='/mnt/6202BA0A02B9E369/ScanNet/scans',
                        help='Directory to dataset')
    parser.add_argument('--train', action='store_true', help='collect training data or teating data')
    args = parser.parse_args()

    DATA_PATH = args.data_path
    DST_PATH = os.path.join(ROOT_DIR, 'datasets/ScanNet')
    if args.train == True:
        SAVE_PATH = os.path.join(DST_PATH, 'scenes', 'train_data') # train / test data
    else:
        SAVE_PATH = os.path.join(DST_PATH, 'scenes', 'test_data')
    ...
    # record number of points for each class
    class_point_dict = {i: 0 for i in range(len(CLASS_NAMES))}
    print(class_point_dict)

    # define val_list
    val_list = [x.rstrip() for x in open(os.path.join(meta_path, 'scannetv2_val.txt'))]
    ...
    if args.train == True:
        for scene_path in scene_paths: # scene_path: /mnt/6202BA0A02B9E369/ScanNet/scans/scene0000_00
            scene_name = os.path.basename(scene_path) # scene0000_00
            if scene_name in val_list: # skip val scene
                continue
        ...
        for scene_path in scene_paths: # scene_path: /mnt/6202BA0A02B9E369/ScanNet/scans/scene0000_00
            scene_name = os.path.basename(scene_path) # scene0000_00
            if scene_name not in val_list: # only collect val scene
                continue
