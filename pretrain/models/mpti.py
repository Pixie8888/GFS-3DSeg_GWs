...
class MultiPrototypeTransductiveInference(nn.Module):
    ...
    def forward(self, support_x, support_y, query_x, query_y):
        ...
        support_feat = self.getFeatures(support_x) # (self.n_way*self.k_shot, 192, 2048)
        ...
        fg_mask = support_y
        bg_mask = torch.logical_not(support_y) # 1 for bg point
        ...
        self.num_prototypes = prototypes.shape[0] # (class+bg) * num_prototype
    ...
    def getForegroundPrototypes(self, feats, masks, k=100):
        ...
        for i in range(self.n_way):
            ...
            feat = feat[index] # get fg points feature
        ...
        prototypes = torch.cat(prototypes, dim=0) # (num_proto * classes, feat_dim)
        labels = torch.cat(labels, dim=0) # (number_proto* class, class+1)
